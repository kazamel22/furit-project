// =====================================
//    Header and Footer Controll Area
// =====================================

// Selecting elements for header section effects
...
window.onscroll = () => {
    ...
    parallax.style.backgroundPositionY = scrollTop * 0.7 + 'px'; // Parallax effect
    home.style.top = 50 - percentage * 1.8 + '%'; // Home text effect
...
// Selecting elements for navigation
let body = document.querySelector('body');
let navItems = document.querySelectorAll('.navbar > li > a');
let footerMenu = document.querySelectorAll('.footer-menu > .fmenu > p > a');
...
// Function for navigation menu (navbar & footer)
function navigate(menuItem) {
    if (menuItem === 'About') {
        services.classList.remove('menu-active');
        pricing.classList.remove('menu-active');
        ...
        body.style.overflow = 'hidden';
    } else if (menuItem === 'Pricing') {
        about.classList.remove('menu-active');
        services.classList.remove('menu-active');
        contact.classList.remove('menu-active');

        pricing.classList.add('menu-active');
...
for (let i = 0; i < navItems.length; i++) {
    navItems[i].addEventListener('click', function () {
        let navItem = navItems[i].getAttribute('data-navitem');
        navigate(navItem);
    })
}

// Footer menu items navigation
for (let i = 0; i < footerMenu.length; i++) {
    footerMenu[i].addEventListener('click', function () {
        let footerMenuItem = footerMenu[i].innerHTML;
        navigate(footerMenuItem);
    })
}

// Selecting elements for menu toggle
let toggleBar = document.querySelector('#toggle-bar');
let navigationArea = document.querySelector('.navbar');
...
// =========================================
//    Header and Footer Controll Area End
// =========================================
...
// Selecting elements for countdown timer
...
let timeCount = setInterval(() => {
    ...
    let eventTime = new Date(`${month[monthIndex]} 18, ${year} 20:10:00`).getTime();
